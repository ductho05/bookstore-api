...
const { bankName } = require("../dtos/ShipperDTO");
...
class Validator {
  ...
  userValidator = Joi.object({
    ...
    phoneNumber: Joi.string()
      ...
      .required(),
    birth: Joi.string()
      .pattern(new RegExp(/([12]\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01]))/))
      .required(),
    email: Joi.string().email().max(225).required(),
    password: Joi.string()
      .pattern(
  ...
  userUpdateValidator = Joi.object({
    ...
    isLock: Joi.boolean(),
    bankName: Joi.string().trim().min(3).max(10),
    bankNumber: Joi.string().trim().min(10).max(20),
    bankUser: Joi.string().trim().min(3).max(255),
  ...
  notificationValidator = Joi.object({
    ...
    url: Joi.string()
      .pattern(
        new RegExp(/[(http(s)?):\/\/(www\.)?a-zA-Z0-9@:%._\+~#=]{2,256}/)
      )
      .required(),
  ...
  orderUpdateValidator = Joi.object({
    ...
    user: Joi.string().pattern(new RegExp(/^[a-fA-F0-9]{24}$/)),
    shipper: Joi.string().pattern(new RegExp(/^[a-fA-F0-9]{24}$/)),
    phone: Joi.string().pattern(
      new RegExp(/^[+]*[(]{0,1}[0-9]{1,4}[)]{0,1}[-\s\./0-9]*$/)
    ),
  });
  ...
  productUpdateValidator = Joi.object({
    ...
    published_date: Joi.string().pattern(
      new RegExp(/([12]\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01]))/)
    ),
    price: Joi.number().min(1),
