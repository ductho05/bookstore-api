...
const UserService = require("../services/ShipperService");
...
class ShipperController {
  async forgetPassword(req, res) {
    ...
    if (error) {
      ...
      const response = await UserService.forgetPasswordShipper(
        value.email,
        value.password
      );
  ...
  //     res.status(400).json(new Response(Status.ERROR, error.message));
  //   } else {
  //     const response = await UserService.loginWithFacebook(
  //       email,
  //       username,
  //       image,
  //       faceId
  ...
  async active(req, res) {
    ...
    if (error) {
      ...
      const response = await UserService.activeShipper(value);

      res
        .status(response.statusCode)
        .json(new Response(response.status, response.message));
    }
  }
  ...
  async getAllUser(req, res) {
    ...
    response =
      page && limit
        ? await UserService.getAllPagination(page, limit)
  ...
  async getAllUserByTime(req, res) {
    ...
    const response = await UserService.getAllByTime(
      page,
      limit,
      firstTime,
      lastTime
  ...
  async insertUser(req, res) {
    const { error, value } = Validator.shipperValidator.validate(req.body);
    // const error = null;
    const file = req.file;
  ...
  //   console.log(id, tas, action);

  //   const response = await UserService.updateTas(id, tas, action);
